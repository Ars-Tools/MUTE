...
extension GenTest {
    ...
    func ps() async throws {
        try await scenario(time: .seconds(30)) {
            ...
            x.maximize(dB: -6)
            let y = pitchshift(x, rate: 1.0 / 0.5)
...
//            let a = filter(z, apf: (lag: 0.3, 0.9))
    ...
    func apf() async throws {
        try await scenario(time: .seconds(120)) {
            ...
            let y = filter(buffer(x)[t], apf: (0.6, 0.9))
    ...
    func sr() async throws {
        try await scenario(time: .seconds(60 * 11)) {
            let x = try buffer(Playback(path: .init(filePath: "/tmp/audio.wav"), loop: true))
