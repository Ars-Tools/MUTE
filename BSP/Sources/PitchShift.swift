...
enum PitchShift {
    @inlinable
    static func PV(length: Int,
                   object: UnsafePointer<bdft_t>,
                   window: UnsafePointer<Float64>,
                   masker: UnsafePointer<Float64>,
                   source: UnsafePointer<Float64>,
                   target: UnsafeMutablePointer<Float64>,
                   memory: UnsafeMutablePointer<Float64>,
                   time: UnsafeMutablePointer<Complex128>,
                   freq: UnsafeMutablePointer<Complex128>) {
        // radius
        vDSP_vmulD(window, 1,
                   source, 1,
                   .init(.init(time)).advanced(by: 0), 2, .init(length))
        vDSP_vclrD(.init(.init(time)).advanced(by: 1), 2, .init(length))
        bdft_forward(object, .init(time), .init(freq))
        vDSP_polarD(.init(.init(freq)), 2, .init(.init(freq)), 2, .init(length))
        cblas_dcopy(length, .init(.init(freq)), 2, memory, 1)
        ...
        bdft_forward(object, .init(time), .init(freq))
        vDSP_polarD(.init(.init(freq)), 2, .init(.init(freq)), 2, .init(length))
        cblas_dcopy(length, memory, 1, .init(.init(freq)), 2)
        
        // synth
        vDSP_rectD(.init(.init(freq)), 2, .init(.init(freq)), 2, .init(length))
        bdft_inverse(object, .init(freq), .init(time))
        vDSP_vmmaD(.init(.init(time)), 2,
                   window, 1,
                   target, 1,
                   masker, 1,
                   target, 1,
            ...
            withUnsafeTemporaryAllocation(of: Float64.self, capacity: ( 3 * stream + 4 ) * period) {
                ...
                for cursor in cursor {
                    ...
                    vDSP_vrampD(withUnsafePointer(to: Float64(cursor), \.self),
                                ...
                                λ, 1, .init(period))
                    // radius
                    i.read(cursor: λ, length: period, target: z.realp, stride: period)
