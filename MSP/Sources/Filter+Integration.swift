...
prefix operator ∫
prefix func ∫(x: Stream) -> some Stream { // cumsum(f(t)) without reset trigger
	filter(x, cascade: .raw(b₀: 0.5, b₁: 0.5, b₂: 0, a₁: -1, a₂: 0))
}
@usableFromInline
enum Integration {
	@usableFromInline
...
extension Integration.Ar: Stream {
	@inlinable
	var count: Int {
	...
	func callAsFunction(interval: CMTime, capacity: Int, resource: inout Resource) throws -> @Sendable (CMTime, Int, UnsafeMutablePointer<Float64>, Int) -> Void {
		...
		let yk = try reset(interval: interval, capacity: capacity, resource: &resource)
				...
				withUnsafeTemporaryAllocation(of: Float64.self, capacity: yc * length) {
					...
					status.withLock {
						for (offset, element) in $0.indices.enumerated() {
							assert(0..<xc ~= offset)
							calculus_integration(result.advanced(by: offset * stride),
			...
			return { moment, length, result, stride in
				...
				withUnsafeTemporaryAllocation(of: Float64.self, capacity: xc * length) {
					...
					status.withLock {
						for (offset, element) in $0.indices.enumerated() {
							assert(0..<yc ~= offset)
							calculus_integration(source.advanced(by: offset * xs),
												 result.advanced(by: offset * stride),
												 result.advanced(by: offset * stride),
												 &$0[element], length)
						}
			...
			return { moment, length, result, stride in
				...
				withUnsafeTemporaryAllocation(of: Float64.self, capacity: ( xc + yc ) * length) {
					...
					status.withLock {
						for (offset, element) in $0.indices.enumerated() {
							assert(0..<zc ~= offset)
							calculus_integration(x.advanced(by: offset * xs),
...
public func Σₜ(_ x: Stream, reset: Stream = φ(count: 1)) -> some Stream {
	Integration.Ar(operand: x, reset: reset)
}
