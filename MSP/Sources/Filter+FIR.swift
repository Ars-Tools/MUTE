...
enum FIR {
	@usableFromInline
	struct Kr<Kernel: Collection<Float64>, Filter: Publisher<(Int, Kernel), Never> & Sendable> {
...
extension FIR.Kr: Stream {
	...
	func callAsFunction(interval: CMTime, capacity: Int, resource: inout Resource) throws -> @Sendable (CMTime, Int, UnsafeMutablePointer<Float64>, Int) -> Void {
		let source = try stream(interval: interval, capacity: capacity, resource: &resource)
		let fr = extent.x
		let fc = extent.y
		let sr = stream.count
		switch domain {
				...
				case 0..<fr:
					buffer.withLock {
						$0.withUnsafeMutablePointer {
							...
							buff[buff.update(fromContentsOf: value)...].update(repeating: .zero)
...
public func filter(_ source: Stream, with kernel: some Publisher<(Int, some Collection<Float64>), Never> & Sendable, extent: SIMD2<Int>, domain: Domain = .time) -> some Stream {
	FIR.Kr(stream: source, filter: kernel, extent: extent, domain: domain)
}
public func filter(_ source: Stream, with kernel: some Collection<some Collection<Float64>>, domain: Domain = .time) -> some Stream {
	filter(source, with: kernel.enumerated().publisher.map(\.self), extent: .init(kernel.count, kernel.map(\.count).max() ?? 1), domain: domain)
}
public func filter(_ source: Stream, with kernel: some Collection<Float64>, domain: Domain = .time) -> some Stream {
