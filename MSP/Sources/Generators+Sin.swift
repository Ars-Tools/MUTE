	...
	func callAsFunction(interval: CMTime, capacity: Int, resource: inout Resource) throws -> @Sendable (CMTime, Int, UnsafeMutablePointer<Float64>, Int) -> Void {
		...
		let ratio = ratio.sink { index, value in
			factor.withLock {
				...
				case ($0.0.indices, $0.1.indices):
					let e = __sincospi_stret(2 * value)
		...
		case ...8:
			return {
				...
				var phasor = UnsafeMutableBufferPointer(start: $2.advanced(by: ( count - 1 ) * $3), count: $1)
				...
				for (offset, (r, θ, ω)) in zip(r, θ, ω).enumerated().reversed() {
					var result = UnsafeMutableBufferPointer(start: $2.advanced(by: offset * $3), count: $1)
					vDSP.add(multiplication: (phasor, 2 * .pi * ω), -θ, result: &result)
					vForce.cos(result, result: &result)
					vDSP.multiply(r, result, result: &result)
				}
			}
		default:
