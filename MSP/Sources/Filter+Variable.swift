...
extension StateVariableFilter.Kr: Stream {
	@usableFromInline
	var count: Int {
		source.count * 3
	}
...
extension StateVariableFilter.Ar: Stream {
	@usableFromInline
	var count: Int {
	...
	func callAsFunction(interval: CMTime, capacity: Int, resource: inout Resource) throws -> @Sendable (CMTime, Int, UnsafeMutablePointer<Float64>, Int) -> Void {
		...
		let yk = try cutoff(interval: interval, capacity: capacity, resource: &resource)
		let zk = try factor(interval: interval, capacity: capacity, resource: &resource)
		let xc = source.count
		let yc = cutoff.count
		let zc = factor.count
...
public func filter(_ source: Stream, svf: (ω₀: Frequency, quality: Float64)) -> some Stream {
	StateVariableFilter.Kr(source: source,
						   cutoff: repeatElement(svf.ω₀, count: source.count).enumerated().publisher.map(\.self),
						   factor: repeatElement(svf.quality, count: source.count).enumerated().publisher.map(\.self))
}
public func filter(_ source: Stream, svf: (ω₀: Stream, quality: Stream)) -> some Stream {
	StateVariableFilter.Ar(source: source, cutoff: svf.ω₀, factor: svf.quality)
}
