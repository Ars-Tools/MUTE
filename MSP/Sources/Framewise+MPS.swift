...
@preconcurrency import class MetalPerformanceShaders.MPSMatrix
@preconcurrency import class MetalPerformanceShaders.MPSMatrixCopy
@preconcurrency import class MetalPerformanceShaders.MPSMatrixDescriptor
...
extension FramewiseMPS.Ne: Stream {
	@inlinable
	var count: Int {
		output
	}
			...
			let source = withUnsafeTemporaryAllocation(of: Float64.self, capacity: matrix.i.rows * length) {
				...
				return buffer.withLock {
					$0.withUnsafeMutablePointer {
						...
						for target in Swift.stride(from: 0, to: matrix.o.rows * period, by: period).lazy.map(ob.advanced(by:)) {
			...
			autoreleasepool {
				...
				for offset in Swift.stride(from: lower, to: upper, by: stride) {
					guard
						...
						os_log(.error, "allocation failed for mpsmatrix")
						assertionFailure()
						continue
					}
					...
					ioproc(value, .init(buffer: i, descriptor: matrix.i), .init(buffer: o, descriptor: matrix.o)) {
						...
						case.none:
							...
							buffer.withLock { $0.withUnsafeMutablePointer {
								for (source, target) in zip(
									Swift.stride(from: 0, to: matrix.o.rows * matrix.o.rowBytes, by: matrix.o.rowBytes).lazy.map(o.contents().advanced(by:)),
						...
						case.some(let error):
							os_log(.error, "%{public}@", String(describing: $0))
...
public func framewise(_ source: Stream, window: WindowDesign, stride: WindowStride, output: Int, device: MTLDevice, ioproc: @escaping@Sendable(CMTime, MPSMatrixDescriptor, MPSMatrixDescriptor) throws -> @Sendable (CMTime, MPSMatrix, MPSMatrix, @escaping(Optional<Swift.Error>) -> Void) -> Void) -> some Stream{
	FramewiseMPS.Ne(source: source, window: window, stride: stride, output: output, device: device, ioproc: ioproc)
}
public func framewise(_ source: Stream, window: WindowDesign, stride: WindowStride, output: Int, device: MTLDevice, ioproc: @escaping@Sendable(CMTime, MPSMatrix, MPSMatrix, (Optional<Swift.Error>) -> Void) -> Void) -> some Stream {
	FramewiseMPS.Ne(source: source, window: window, stride: stride, output: output, device: device) { _, _, _  in
		ioproc
	}
}
public func framewise(_ source: Stream, window: WindowDesign, stride: WindowStride, output: Int, thread: MTLCommandQueue, tensor: @escaping@Sendable(CMTime, MPSGraph, MPSGraphTensor, MPSGraphTensor) -> MPSGraphTensor) -> some Stream {
	FramewiseMPS.Ne(source: source, window: window, stride: stride, output: output, device: thread.device) {
		let g = MPSGraph()
		let t = g.placeholder(shape: .none, dataType: .float32, name: .none)
		let x = g.placeholder(shape: [$1.rows, $1.columns].map(NSNumber.init), dataType: $1.dataType, name: .none)
