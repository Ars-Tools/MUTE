...
extension FramewiseTime.Ne: Stream {
	@inlinable
	var count: Int {
		output
	}
	...
	func callAsFunction(interval: CMTime, capacity: Int, resource: inout Resource) throws -> @Sendable (CMTime, Int, UnsafeMutablePointer<Float64>, Int) -> Void {
		...
		let buffer = Autorelease.Buffer<Float64>(count: stream.o * period + stream.i * (period + capacity))
		return { moment, length, target, bounds in
			withUnsafeTemporaryAllocation(of: Float64.self, capacity: max((stream.o + stream.i) * window.count, stream.i * length)) {
				...
				let layout = zip(Swift.stride(from: 0, through: (stream.o + stream.i) * window.count, by: window.count).lazy.map(source.advanced(by:)), sequence(first: window.count, next: \.self)).map(UnsafeMutableBufferPointer.init)
...
public func framewise(_ source: Stream, window: WindowDesign, stride: WindowStride, output: Int, ioproc: @escaping@Sendable(CMTime, Int) throws -> @Sendable (CMTime, ArraySlice<UnsafeMutableBufferPointer<Float64>>, ArraySlice<UnsafeMutableBufferPointer<Float64>>) -> Void) -> some Stream {
	FramewiseTime.Ne(source: source, window: window, stride: stride, output: output, ioproc: ioproc)
}
public func framewise(_ source: Stream, window: WindowDesign, stride: WindowStride, output: Int, ioproc: @escaping@Sendable(CMTime, ArraySlice<UnsafeMutableBufferPointer<Float64>>, ArraySlice<UnsafeMutableBufferPointer<Float64>>) -> Void) -> some Stream {
	FramewiseTime.Ne(source: source, window: window, stride: stride, output: output) { _, _ in ioproc }
}
