	...
	func callAsFunction(interval: CMTime, capacity: Int, instance: inout Instance) throws -> @Sendable (CMTime, Int, UnsafeMutablePointer<Float64>, Int) -> Void {
		...
		case xc:
			let filter = repeatElement(order, count: xc).map {
				Autorelease.Object(object: lms_filter_create($0)) {
					lms_filter_destroy($0)
				}
			}
			let cancel = signal.sink {
				switch $0 {
				case filter.indices:
					lms_filter_mu(filter[$0].reference, $1)
				default:
			...
			return { [cancel] moment, length, target, xs in
				...
				withUnsafeTemporaryAllocation(of: Float64.self, capacity: yc * length) {
					...
					for (offset, filter) in filter.enumerated().reversed() {
						lms_filter_error(filter.reference,
...
public func residual(target: Stream, source: Stream, sgd order: Int, μ: some Publisher<(Int, Float64), Never> & Sendable) -> some Stream {
	LMS.Residual(target: target, source: source, signal: μ, order: order)
}
public func residual(target: Stream, source: Stream, sgd order: Int, μ: Float64) -> some Stream {
	residual(target: target, source: source, sgd: order, μ: `repeat`(μ, count: broadcast(x: source.count, y: target.count)))
}
