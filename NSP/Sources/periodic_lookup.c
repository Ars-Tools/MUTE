...
// like a comb filter
void periodic_lookup_with_update(register double const * __nonnull x, // input
                                 register double       * __nonnull y, // output
                                 register double       * __nonnull z, // buffer
                                 register double const * __nonnull w, intptr_t const ldw,
                                 register intptr_t       cursor,
                                 register intptr_t const offset,
                                 register intptr_t const period,
                                          intptr_t const length) {
    for ( register intptr_t const _ = cursor + length ; cursor < _ ; ++ cursor, ++ x, ++ y, w += ldw )
        z[(cursor+offset)%period] = *y = fma(z[(cursor)%period], *w, *x);
}
