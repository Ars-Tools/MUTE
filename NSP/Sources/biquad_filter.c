...
biquad_filter_t * __nonnull const biquad_filter_create(intptr_t const c) {
	void * const p = __malloc__(sizeof(biquad_filter_t const) + c * sizeof(simd_double4 const));
...
__attribute__((always_inline))
inline void biquad_filter_convolve_active(register double const * __nonnull b, register intptr_t const ldb,
...
__attribute__((always_inline))
inline void biquad_filter_convolve_static(register simd_double3 const b,
