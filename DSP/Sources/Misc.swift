...
@inlinable@inline(__always)@_transparent
func move(x: UnsafePointer<Float64>, ldx: Int,
          y: UnsafeMutablePointer<Float64>, ldy: Int,
          rows: Int, cols: Int) {
    for (x, y) in zip(stride(from: 0, to: rows * ldx, by: ldx).lazy.map(x.advanced(by:)),
                      stride(from: 0, to: rows * ldy, by: ldy).lazy.map(y.advanced(by:))) {
        memmove(y, x, cols * MemoryLayout<Float64>.stride)
    }
}
@inlinable@inline(__always)@_transparent
func zero(x: UnsafeMutablePointer<Float64>, ldx: Int,
          rows: Int, cols: Int) {
    for x in stride(from: x, to: x.advanced(by: rows * ldx), by: ldx) {
        vDSP_vclrD(x, 1, .init(cols))
    }
}
@inlinable@inline(__always)@_transparent
...
@inlinable@inline(__always)@_transparent@_disfavoredOverload
func fold<T>(start: UnsafePointer<T>, count: Int, stream: Int, period: Int) -> Array<UnsafeBufferPointer<T>> {
    zip(stride(from: start, to: start.advanced(by: stream * period), by: period), repeatElement(count, count: stream))
        .map(UnsafeBufferPointer<T>.init(start:count:))
}
@inlinable@inline(__always)@_transparent
func fold<T>(start: UnsafePointer<T>, count: Int, stream: Int, period: Int) -> LazyMapSequence<Zip2Sequence<StrideTo<UnsafePointer<T>>, Repeated<Int>>, UnsafeBufferPointer<T>> {
    zip(stride(from: start, to: start.advanced(by: stream * period), by: period), repeatElement(count, count: stream)).lazy
        .map(UnsafeBufferPointer<T>.init(start:count:))
}
@inlinable@inline(__always)@_transparent@_disfavoredOverload
func fold<T>(start: UnsafeMutablePointer<T>, count: Int, stream: Int, period: Int) -> Array<UnsafeMutableBufferPointer<T>> {
    zip(stride(from: start, to: start.advanced(by: stream * period), by: period), repeatElement(count, count: stream))
        .map(UnsafeMutableBufferPointer<T>.init(start:count:))
}
@inlinable@inline(__always)@_transparent
func fold<T>(start: UnsafeMutablePointer<T>, count: Int, stream: Int, period: Int) -> LazyMapSequence<Zip2Sequence<StrideTo<UnsafeMutablePointer<T>>, Repeated<Int>>, UnsafeMutableBufferPointer<T>> {
    zip(stride(from: start, to: start.advanced(by: stream * period), by: period), repeatElement(count, count: stream)).lazy
        .map(UnsafeMutableBufferPointer<T>.init(start:count:))
}
@inlinable@inline(__always)@_transparent@_disfavoredOverload
func fold<T>(start: UnsafeMutableBufferPointer<T>, count: Int, stream: Int, period: Int) -> Array<UnsafeMutableBufferPointer<T>> {
    stride(from: 0, to: stream * period, by: period)
        .map { start.extracting($0..<$0+count) }
}
@inlinable@inline(__always)@_transparent
func fold<T>(start: UnsafeMutableBufferPointer<T>, count: Int, stream: Int, period: Int) -> LazyMapSequence<StrideTo<Int>, UnsafeMutableBufferPointer<T>> {
    stride(from: 0, to: stream * period, by: period).lazy
        .map { start.extracting($0..<$0+count) }
}
@inlinable@inline(__always)@_transparent@_disfavoredOverload
